 ...
 */
public final class IDworkers implements Comparable<IDworkers>, Serializable {

    private static final long serialVersionUID = 3670079982654483072L;

    static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(IDworkers.class);
     ...
     * ObjectIds this way, but it does not match the
     * <a href="http://docs.mongodb.org/manual/reference/object-id/">ObjectId specification</a>, which
     ...
     * @since 2.12.0
     */
    public static IDworkers createFromLegacyFormat(final int time, final int machine,
        final int inc) {
        return new IDworkers(time, machine, inc);
    }
     ...
     * @param date the date
     */
    public IDworkers(final Date date) {
     ...
     */
    public IDworkers(final Date date, final int counter) {
     ...
     * @param counter           the counter
     * @throws IllegalArgumentException if the high order byte of machineIdentifier or counter is not
     *                                  zero
     */
    public IDworkers(final Date date, final int machineIdentifier, final short processIdentifier,
        final int counter) {
     ...
     *                                  zero
     */
    public IDworkers(final int timestamp, final int machineIdentifier,
        final short processIdentifier, final int counter) {
        this(timestamp, machineIdentifier, processIdentifier, counter, true);
    }

    private IDworkers(final int timestamp, final int machineIdentifier,
        final short processIdentifier, final int counter, final boolean checkCounter) {
     ...
     *                                  ObjectId
     */
    public IDworkers(final String hexString) {
     ...
     */
    public IDworkers(final byte[] bytes) {
        if (bytes == null) {
            throw new IllegalArgumentException();
        }
        if (bytes.length != 12) {
     ...
     */
    IDworkers(final int timestamp, final int machineAndProcessIdentifier, final int counter) {
    ...
    @Override public boolean equals(final Object o) {
        ...
        IDworkers objectId = (IDworkers) o;
    ...
    @Override public int compareTo(final IDworkers other) {
        if (other == null) {
    ...
    private static int createMachineIdentifier() {
        ...
        try {
            ...
            // LOGGER.log(Level.WARNING, "Failed to get machine identifier from network interface, using
    ...
    private static short createProcessIdentifier() {
        ...
        try {
            ...
            if (processName.contains("@")) {
                processId =
                    (short) Integer.parseInt(processName.substring(0, processName.indexOf('@')));
            } else {
                processId =
                    (short) java.lang.management.ManagementFactory.getRuntimeMXBean().getName()
                        .hashCode();
            }

        } catch (Throwable t) {
            processId = (short) new SecureRandom().nextInt();
            // LOGGER.log(Level.WARNING, "Failed to get process identifier from JMX, using random number
            // instead", t);
        }
