...
- OSR编译：
    osr编译只是一种手段，不是目的。
    参考该回答：https://www.zhihu.com/question/45910849
...
- 虚方法分派的实现机理：
    ...
    - 总结：针对于方法，从动态和静态角度来看分为两种，一种是用 invokestatic 调用的静态方法和用 invokespecial 调用的构造函数和私有方法（因为在编译期改方法就已经确定了），另外一种是 invokevirtual 调用的方法,jvm单独为其维护一个虚方法表，记录重写或者实现的方法，果子类重写了父类中某个方法的代码，则该方法第一次出现的位置的索引更换到子类的实现代码上，而不会在方法表中出现新的项，在进行具体调用的时候是通过索引进行调用，而不是采用元数据中的方法引用直接进行调用。
