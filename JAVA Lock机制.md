...
1. CAS机器级的原语；
2. LockSupport.unpark(s.thread）唤醒线程，LockSupport.unpark(）禁止线程；
3. 链表的数据结构。
4. AQS的线程入队和出对唤醒粗略
...
public boolean release(int arg){
   更新同步器的状态
   if(tryRelease(arg)方法进行当前状态是否可以进行释放锁的操作){
     // 释放一个线程或者多个线程
   }
}
...
```
     ...
     * 将线程加入到线程队列之后，根据状态来决定当前线程时一直进行轮询，并park住改线程，如果锁释放的时候，会唤醒该线程，继续执行循环，尝试获取锁，失败了，就继续park住。
     * @param node the node
...
#### ReentrantLock
---
独占的同步锁,支持公平和非公平的策略机制：

1. 公平: Threads acquire a `fair` lock in the order in which they requested it
；改策略下的锁竞争机制具有公平性，既先到的线程请求也是先获得锁，由线程等待队列来维护其顺序性；
2. 非公平: a `nonfair` lock permits barging: threads requesting a lock can jump ahead of the 
queue of waiting threads if the lock happens to be available when it is requested;
非公平锁比较粗鲁，上来就直接尝试占有锁，如果尝试失败，就再采用类似公平锁那种方式.
 

在实现过程中，采用了CAS底层机器原语来支持可见性和原子性。
...
```
    ...
    public final void acquire(int arg) {
        // 首先判断是否判断当前的状态是否可以获得锁操作，如果可以的话：线程正常执行
        // 如果不行的话，就将当前线程加入等待队列，新增节点在对尾并指定节点正在以独占模式等待的标记（Node.EXCLUSIVE）
        // 以线程已经在队列中的独占轮询获取。 由条件tryAcquire条件来判断是否可以获取使用锁。
        if (!tryAcquire(arg) &&
    ...
    // 释放锁对象
        if (tryRelease(arg)) {//尝试是否可以进行锁释放操作
            Node h = head;
            if (h != null && h.waitStatus != 0)//如果线程节点状态是需要被唤醒的。
                unparkSuccessor(h);//唤醒线程
            return true;
        }
        return false;//尝试释放锁操作失败，返回false
    }
     ...
     */
    //在共享模式下尝试获得锁操作，如果线程时中断的，那就中断该线程，否则就进行锁获取操作，如果成功了，就直接返回，否则，就执行doAcquireSharedInterruptibly（）操作，进行入队操作，可能反复阻塞和解除阻塞，调用tryAcquireShared（int）直到成功或线程中断.
...
```
    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
        ...
        final boolean nonfairTryAcquire(int acquires) {
            ...
            int c = getState();
            //如果当前没有线程占有锁，当前线程直接通过cas指令占有锁，就算自己排在队尾也是这样，就是存在线程跳跃执行
            if (c == 0) {
            ...
            //如果当前线程本身就持有锁，那么叠加状态值，持续获得锁（重入锁）
...
**在该同步器中state用来标记当前获得锁线程的重入数，既获得锁的次数；setExclusiveOwnerThread和getExclusiveOwnerThread是用来操作当前持有锁的线程对象**
...
```
    ...
    static final class NonfairSync extends Sync {
         ...
         */
        final void lock() {
        // 开始尝试获得锁，更新state的状态为1表示该锁已经被占用，如果成功表示该锁已经被占用。
            if (compareAndSetState(0, 1))
                setExclusiveOwnerThread(Thread.currentThread());
            else
        // 否则就去尝试获取锁，这块和公平最大的区别在于：他不需要去判断该线程是否是等待线程的第一个，而是直接去尝试获得锁，如果失败了，那就乖乖的去等待线程中呆着。
                acquire(1);
        }
    ...
    static final class FairSync extends Sync {
        ...
        protected final boolean tryAcquire(int acquires) {
            final Thread current = Thread.currentThread();
            int c = getState();
            // 如何当前state=0说明当前没有线程占有锁
            if (c == 0) {
            // hasQueuedPredecessors（）检验线程队列中是否还有正在等待的线程，如果在当前线程之前有排队的线程，则为true，
            如果当前线程在队列的头部或队列为空，则为false
            //通过CAS更新state为1，表明锁被当前线程占用，并设置当前线程exclusiveOwnerThread
...
#### CountDownLatch
---
CountDownLatch能够使一个线程等待其他线程完成各自的工作后再执行。例如，应用程序的主线程希望在负责启动框架服务的线程已经启动所有的框架服务之后再执行。
在实现层面是通过state代表计数器，只有当state为0的时候，才能获得这把锁，然后继续执行。
...
```
     ...
     */
     共享模式下的锁释放，如果能获取释放锁的操作，则执行释放锁操作，否则，返回失败。
    在共享模式下释放。 通过取消阻止一个或多个线程如果{@link #tryReleaseShared}返回true。
     ...
     */
     释放共享模式的操作 - 信号后继和确保传播。（注意：对于独占模式，只释放金额，如果它需要信号，调用unpark的头的继任者。）
...
#### CyclicBarrier
---
CyclicBarrier类似就是一个拦截器，调用await()的方法的线程，只有到一定数量的线程执行各自的任务，才能继续执行，否则该线程就一直是阻塞状态。
